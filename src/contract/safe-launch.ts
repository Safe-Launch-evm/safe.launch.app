...
import { AbiEvent, Hex, WalletClient, createPublicClient, createWalletClient, getContract, http, parseUnits, zeroAddress } from "viem";
import { assetChainTestnet } from "viem/chains";
import { AddLiquidityAbi, PRICING_CURVE_TARGET, RPC_URL, SAFE_LAUNCH_ADDRESS, getCurrentPrice, getTokenSupplyInPool, getWRWASupplyInPool } from './utils';
...
export default class SafeLaunch {
    ...
    async createToken(name: string, symbol: string, amount: string) {
        try {
            ...
            const estimatedGas = await this.contract.estimateGas.createToken(args, { value: parseUnits(amount, 18) })
    ...
    async buyToken(tokenAddress: string, amount: string) {
        try {
            ...
            const estimatedGas = await this.contract.estimateGas.swap(args, { value: parseUnits(amount, 18) })
            const gasLimit = BigInt(Math.floor(Number(estimatedGas) * 1.1)) // Add 10%
    ...
    async getTokenMarketStats(tokenAddress: string) {
        ...
        return {
            price, circulatingSupply, marketcap, liquidity
        }
    }
